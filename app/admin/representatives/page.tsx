...
import RepresentativesTable from "@/components/admin/representatives-table";
import type { Representative } from "@/lib/db/schema";
import { adminSidebarLinks } from "@/lib/config/admin-sidebar";

/**
 * Página de Gerenciamento de Representantes - Área Administrativa
 * Permite criar, editar e gerenciar representantes
 */
...
export default function AdminRepresentatives() {
  ...
  // Carrega representantes ao montar componente
  useEffect(() => {
        ...
        <div className="p-3 sm:p-6 md:p-10 rounded-tl-2xl border border-neutral-200 bg-white flex flex-col gap-2 sm:gap-3 flex-1 w-full">
          ...
          <div className="mb-3 sm:mb-6">
            ...
            <p className="text-neutral-600 text-sm mt-1">
              Gerencie representantes, clínicas parceiras e empresas que cadastram cupons
            </p>
          </div>
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="text-neutral-500">Carregando representantes...</div>
            </div>
          ) : (
            <RepresentativesTable
              representatives={representatives}
              onRepresentativeCreated={fetchRepresentatives}
              onRepresentativeUpdated={fetchRepresentatives}
              onRepresentativeDeleted={fetchRepresentatives}
            />
          )}
        </div>
      </div>
